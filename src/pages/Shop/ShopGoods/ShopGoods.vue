<template>
  <div>
    <div class="goods">
      <div class="menu-wrapper">
        <ul>
          <!--current-->
          <li
            class="menu-item"
            v-for="(good, index) in goods"
            :key="index"
            :class="{ current: index === currentIndex }"
            @click="clickMenuItem(index)"
          >
            <span class="text bottom-border-1px">
              <img class="icon" v-lazy="good.icon" v-if="good.icon" />
              {{ good.name }}
            </span>
          </li>
        </ul>
      </div>
      <div class="foods-wrapper">
        <ul ref="foodsUl">
          <li
            class="food-list-hook"
            v-for="(good, index) in goods"
            :key="index"
          >
            <h1 class="title">{{ good.name }}</h1>
            <ul>
              <li
                class="food-item bottom-border-1px"
                v-for="(food, index) in good.foods"
                :key="index"
                @click="showFood(food)"
              >
                <div class="icon">
                  <img width="57" height="57" v-lazy="food.icon" />
                </div>
                <div class="content">
                  <h2 class="name">{{ food.name }}</h2>
                  <p class="desc">{{ food.description }}</p>
                  <div class="extra">
                    <span class="count">月售{{ food.sellCount }}份</span>
                    <span>好评率{{ food.rating }}%</span>
                  </div>
                  <div class="price">
                    <span class="now">￥{{ food.price }}</span>
                    <span class="old" v-if="food.oldPrice"
                      >￥{{ food.oldPrice }}</span
                    >
                  </div>
                  <div class="cartcontrol-wrapper">
                    <CartControl :food="food" />
                  </div>
                </div>
              </li>
            </ul>
          </li>
        </ul>
      </div>
    </div>
    <Food :food="food" ref="myFood" />
    <div id="shop-cart">
      <ShopCart />
    </div>
  </div>
</template>

<script>
import BScroll from "better-scroll";
import CartControl from "../../../components/CartControl/CartControl";
import ShopCart from "../../../components/ShopCart/ShopCart.vue";
import Food from "../../../components/Food/Food";

import { mapState, mapActions } from "vuex";
import { RECEIVE_SHOP_GOODS } from "../../../store/mutations-types";

export default {
  data() {
    return {
      scrollY: 0,
      tops: [0],
      food: {},
    };
  },
  components: {
    CartControl,
    Food,
    ShopCart,
  },
  computed: {
    ...mapState(["goods"]),
    // 获取当前下标位置
    currentIndex() {
      let { scrollY, tops } = this;
      // 求出对应的内容区域下标
      let index = tops.findIndex((top, index) => {
        // 当前高度大于top高度,且小于下一个top高度则这个就是对应的区域返回true
        return scrollY >= top && scrollY < tops[index + 1];
      });
      // 上面用 index === currentIndex,返回true则当前li添加了类名
      return index;
    },
  },
  methods: {
    ...mapActions([RECEIVE_SHOP_GOODS]),
    // 初始化滚动,scrollY
    _initScroll() {
      this.$nextTick(() => {
        // 初始化
        new BScroll(".menu-wrapper", {
          click: true,
        });
        this.foodsWrapper = new BScroll(".foods-wrapper", {
          probeType: 2,
          click: true,
        });

        // 滑动时触发的事件
        this.foodsWrapper.on("scroll", ({ y }) => {
          this.scrollY = Math.abs(y);
        });

        // 解决惯性问题导致的不更新区块
        this.foodsWrapper.on("scrollEnd", ({ y }) => {
          this.scrollY = Math.abs(y);
        });
      });
    },
    // 初始化tops
    _initTops() {
      // 1.初始化tops
      const tops = [];
      let top = 0;
      tops.push(top);

      // 2.得出所有的li高
      let lis = this.$refs.foodsUl.querySelectorAll(".food-list-hook");
      // 调出slice方法指定给伪数组lis,返回一个真数组,使用forEach进行比那里将数据计算
      Array.prototype.slice.call(lis).forEach((li) => {
        top += li.clientHeight;
        tops.push(top);
      });

      // 更新数据
      this.tops = tops;
    },
    // 菜单列表
    clickMenuItem(index) {
      // 获取当前区块对应的高度
      let scrollY = this.tops[index];
      // 让其更新对应区块,显示高亮
      this.scrollY = scrollY;
      // 右侧的商品栏移动, 参数为: x, y, time
      this.foodsWrapper.scrollTo(0, -scrollY, 300);
    },
    // 点击时传递food
    showFood(food) {
      // 1.修改data里面的food
      this.food = food;
      // 2.显示food
      this.$refs.myFood.toggleShow();
    },
  },
  mounted() {
    if (this.goods.length > 0) {
      this.$nextTick(() => {
        this._initScroll();
        this._initTops();
      });
    } else {
      this[RECEIVE_SHOP_GOODS](() => {
        // 等待页面重新渲染完毕在执行
        this.$nextTick(() => {
          this._initScroll();
          this._initTops();
        });
      });
    }
  },
};
</script>

<style lang="stylus" rel="stylesheet/stylus" scoped>
@import '../../../common/stylus/mixins.styl';

#shop-cart {
  position: fixed;
  bottom: 0;
  left: 0;
  width: 100%;
}

.goods {
  display: flex;
  position: absolute;
  top: 195px;
  bottom: 46px;
  width: 100%;
  background: #fff;
  overflow: hidden;

  .menu-wrapper {
    flex: 0 0 80px;
    width: 80px;
    background: #f3f5f7;

    .menu-item {
      display: table;
      height: 54px;
      width: 56px;
      padding: 0 12px;
      line-height: 14px;

      &.current {
        margin-top: -1px;
        background: #fff;
        color: $green;
        font-weight: 700;

        .text {
          border-none();
        }
      }

      .icon {
        display: inline-block;
        vertical-align: top;
        width: 12px;
        height: 12px;
        margin-right: 2px;
        background-size: 12px 12px;
        background-repeat: no-repeat;
      }

      .text {
        display: table-cell;
        width: 56px;
        vertical-align: middle;
        bottom-border-1px(rgba(7, 17, 27, 0.1));
        font-size: 12px;
      }
    }
  }

  .foods-wrapper {
    flex: 1;

    .title {
      padding-left: 14px;
      height: 26px;
      line-height: 26px;
      border-left: 2px solid #d9dde1;
      font-size: 12px;
      color: rgb(147, 153, 159);
      background: #f3f5f7;
    }

    .food-item {
      display: flex;
      margin: 18px;
      padding-bottom: 18px;
      bottom-border-1px(rgba(7, 17, 27, 0.1));

      &:last-child {
        border-none();
        margin-bottom: 0;
      }

      .icon {
        flex: 0 0 57px;
        margin-right: 10px;
      }

      .content {
        flex: 1;

        .name {
          margin: 2px 0 8px 0;
          height: 14px;
          line-height: 14px;
          font-size: 14px;
          color: rgb(7, 17, 27);
        }

        .desc, .extra {
          line-height: 10px;
          font-size: 10px;
          color: rgb(147, 153, 159);
        }

        .desc {
          line-height: 12px;
          margin-bottom: 8px;
        }

        .extra {
          .count {
            margin-right: 12px;
          }
        }

        .price {
          font-weight: 700;
          line-height: 24px;

          .now {
            margin-right: 8px;
            font-size: 14px;
            color: rgb(240, 20, 20);
          }

          .old {
            text-decoration: line-through;
            font-size: 10px;
            color: rgb(147, 153, 159);
          }
        }

        .cartcontrol-wrapper {
          position: absolute;
          right: 0;
          bottom: 12px;
        }
      }
    }
  }
}
</style>
