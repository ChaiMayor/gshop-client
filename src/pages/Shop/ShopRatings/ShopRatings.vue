...
import { RECEIVE_SHOP_RATINGS } from "../../../store/mutations-types.js";
...
export default {
  ...
  computed: {
    ...
    filterRatings() {
      ...
      // 将filter函数从原型链上面转给 ratings 使用
      return Array.prototype.filter.call(ratings, (rating) => {
        ...
        return (
          // 如果当前是2显示全部, 则不需要判断下一条, 所以数组内容全部挑选出来
          // 如果当前是0|1显示满意,不满意, 将符合0|1的属性的数组挑选出来
          (selectType === 2 || selectType === rateType) &&
          // 如果当前为false, 不只看有内容的评论, 取反则跳过这个判断
          // 如果当前为true, 只看有内容的评论, 判断下条内容字数长度是否大于0, 不为空
